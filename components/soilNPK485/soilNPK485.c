...
const char soilNPK485_modbus_send_data[]={0x01,0x03,0x00,0x1E,0x00,0x03,0x65,0xCD};//发送查询指令
...
void soilNPK485_Read(void)
{

    uint8_t data_u2[BUF_SIZE];
    gpio_set_level(RS485RD, 1); //RS485输出模式
    uart_write_bytes(UART_NUM_2, soilNPK485_modbus_send_data, 8);
    ...
    if(len2!=0)
    {
        ...
        // printf("u25=%x,u26=%x\n",data_u2[7],data_u2[8]);
        if((data_u2[9]*256+data_u2[10])==CRC16_ModBus(data_u2,9))
        {
            soilN_485=data_u2[3]*256+data_u2[4];
            printf("soilN_485=%d mg/kg\n",soilN_485);

            soilP_485=data_u2[5]*256+data_u2[6];
            printf("soilP_485=%d mg/kg\n",soilP_485);

            soilK_485=data_u2[7]*256+data_u2[8];
            printf("soilK_485=%d mg/kg\n",soilK_485);

        }
        else
        {
            printf("soilNPK485 crc error\n");
        }
